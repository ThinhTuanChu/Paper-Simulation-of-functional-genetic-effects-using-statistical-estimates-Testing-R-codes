...
addingpop <- generateStartPop(genodf=geno3way,nIndividuals=nstartpop,infuncParameters='statisticalOne',
...
# setup hai matrices for a1
tempfreqGeno <- freqGenotypes_geno3way[SNPinfo[,4]=='T',]
hai_a1 <- temp1 <- geno3way[,SNPinfo[,4]=='T']
tempfunc <- function(iQtl) {
  hai_a1[temp1[,iQtl]==0,iQtl] <<- findhai(px=tempfreqGeno[iQtl,],allelecount=0)
  hai_a1[temp1[,iQtl]==1,iQtl] <<- findhai(px=tempfreqGeno[iQtl,],allelecount=1)
  hai_a1[temp1[,iQtl]==2,iQtl] <<- findhai(px=tempfreqGeno[iQtl,],allelecount=2)  }
invisible(mapply(tempfunc,iQtl=1:ncol(hai_a1)))    # using apply

# setup hdi matrices
tempfreqGeno <- freqGenotypes_geno3way[SNPinfo[,4]=='T',]
hdi3way <- temp1 <- geno3way[,SNPinfo[,4]=='T']
tempfunc <- function(iQtl) {
  hdi3way[temp1[,iQtl]==0,iQtl] <<- findhdi(px=tempfreqGeno[iQtl,],allelecount=0)
  hdi3way[temp1[,iQtl]==1,iQtl] <<- findhdi(px=tempfreqGeno[iQtl,],allelecount=1)
  hdi3way[temp1[,iQtl]==2,iQtl] <<- findhdi(px=tempfreqGeno[iQtl,],allelecount=2)
}
...
tempfunc <- function(idd) {
  #xAk <- geno3way[idd,episPairLoci[,1]]
  #xAl <- geno3way[idd,episPairLoci[,2]]
  #haihaiVect <- mapply(findhaihai,pk=pxAk,ack=xAk,pl=pxAl,acl=xAl)		
  haihaiVect <- mapply(findhaihai,pk=pxAk,ack=geno3way[idd,episPairLoci[,1]],pl=pxAl,acl=geno3way[idd,episPairLoci[,2]])
...
criteriaAccept <- function(tempSol) {  #tempSol=startpop[1,];tempSol=opt@solution
  ...
  #### calculate statistical additive genetic effects by animals
  idgv <- array(data = 0,dim=c(nAnimal,nTrait)) 
  for (iTrait in 1:(nTrait)) {
    idgv[,iTrait] <- hai_a1 %*% aQtleffects_Stat[,iTrait]
  }
  v_aStat=var(idgv)   # calculate covariances for traits
