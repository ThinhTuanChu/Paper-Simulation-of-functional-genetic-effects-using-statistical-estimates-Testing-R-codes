...
##############################################################################
###### Scenario - Example 2: Individual phenotypes with LD using SF_GA #######
##############################################################################
...
geno3way=genoFounderRes 		# this input is required, but too lazy to change. 
...
for (iSampling in 1:nSamplingRep)  {
  ...
  # variables needed to run optimization algorithm: all global variables + geno3way, nstartpop, maxiterOpt
  # Method of variance calculation by indivdiual or by locus (Refer to the paper for the method)

  filename <- paste0(programdir,"S3GAopt4_bylocus.R")   # 'S3GAopt3_byid.R'
  if (!file.exists(filename)) {  write(paste('Error: The file',filename,'does not exists. This module is needed for',filename),file=outlog,append=file.exists(outlog))}
  source(filename)
