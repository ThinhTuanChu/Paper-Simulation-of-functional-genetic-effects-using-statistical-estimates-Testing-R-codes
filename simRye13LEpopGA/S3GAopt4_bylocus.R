...
if (!exists('geno3way')) stop (paste('geno3way is required, but missing.'))
  ...
  for (idstartpop in 1:nIndividuals) {  
    ...
    if (domsimulated) {
      ...
      if (!is.null(domcovarTraits)) {
        ...
        invisible(mapply(rescaleFun,iQtl=1:nQtl)) 
      }
    }
...
addingpop <- generateStartPop(genodf=geno3way,nIndividuals=nstartpop,infuncParameters='statisticalOne',methodCalculateVar='byid',
...
# setup hx_d1 & px_d1 for calculation of variances by locus for dominance
px_d1 <- freqGenotypes_geno3way[SNPinfo[,4]=='T',]
px <- lapply(seq_len(nrow(px_d1)), function(i) px_d1[i,])
hx_d1 <- array(data = 0,dim=c(nQtl,3))
hx_d1[,1] <- mapply(findhdi,px=px,allelecount=0)
hx_d1[,2] <- mapply(findhdi,px=px,allelecount=1)
hx_d1[,3] <- mapply(findhdi,px=px,allelecount=2)

# setup hk_aa1 & hl_aa1  for calculation of variances by locus for epistasis
hk_aa1 <- hl_aa1 <- array(data = 0,dim=c(nEpisPairLoci,3))
temp1 <- freqGenotypes_geno3way[episPairLoci[,1],]
px <- lapply(seq_len(nrow(temp1)), function(i) temp1[i,])
hk_aa1[,1] <- mapply(findhai,px=px,allelecount=0)
temp1 <- freqGenotypes_geno3way[episPairLoci[,1],]
px <- lapply(seq_len(nrow(temp1)), function(i) temp1[i,])
hk_aa1[,2] <- mapply(findhai,px=px,allelecount=1)
temp1 <- freqGenotypes_geno3way[episPairLoci[,1],]
px <- lapply(seq_len(nrow(temp1)), function(i) temp1[i,])
hk_aa1[,3] <- mapply(findhai,px=px,allelecount=2)

temp1 <- freqGenotypes_geno3way[episPairLoci[,2],]
px <- lapply(seq_len(nrow(temp1)), function(i) temp1[i,])
hl_aa1[,1] <- mapply(findhai,px=px,allelecount=0)
temp1 <- freqGenotypes_geno3way[episPairLoci[,2],]
px <- lapply(seq_len(nrow(temp1)), function(i) temp1[i,])
hl_aa1[,2] <- mapply(findhai,px=px,allelecount=1)
temp1 <- freqGenotypes_geno3way[episPairLoci[,2],]
px <- lapply(seq_len(nrow(temp1)), function(i) temp1[i,])
hl_aa1[,3] <- mapply(findhai,px=px,allelecount=2)

# setup bklOther
bklOther <- array(data = NA,dim=c(6,6,nEpisPairLoci))
Wfkl <- setupWfklmat1()  # put here so code more efficient
tempfunc <- function(pairno) {
  #pairno=1
  pk=freqGenotypes_geno3way[episPairLoci[pairno,1],]
  pl=freqGenotypes_geno3way[episPairLoci[pairno,2],]
  Wskl <- setupWsklmat(pk,pl)
  temp1 <- t(Wskl)%*%Wskl
  if (det(temp1)<0.0000000001) diag(temp1)=diag(temp1)+0.00000001   # this might be needed for non-invertable matrix
...
varvectExpected <- c(diag(aRes_covar),diag(aaNRMSRes_covar),diag(dNRMSRes_covar))
...
criteriaAccept <- function(tempSol) {  #tempSol=startpop[1,];tempSol=opt@solution
  ...
  aQtleffects_Stat <- domValues_Stat <- array(data = 0,dim=c(nQtl,nTrait))  
  ...
  ### calculate statistical variances of additive
  varbylocus <- array(data = 0,dim=c(nTrait,nTrait,nQtl))
  tempfunc <- function(itemp) {
    varbylocus[,,itemp] <<- calculateVarbyLocus(EffInput=aQtleffects_Stat[itemp,],px=px_a1[itemp,],
                                                hx=hx_a1[itemp,])
  }
  invisible(mapply(tempfunc,itemp=1:nQtl))    # using apply
  v_temp=array(data = 0,dim=c(nTrait,nTrait))
  ...
  tempfunc <- function(itemp) {
    varbylocus[,,itemp] <<- calculateVarbyLocusEpis(EffInput=episPairLociEffect_Stat[itemp,],
                                                    pk=freqGenotypes_geno3way[episPairLoci[itemp,1],],pl=freqGenotypes_geno3way[episPairLoci[itemp,2],],
                                                    hk=hk_aa1[itemp,],
                                                    hl=hl_aa1[itemp,])
  }
  invisible(mapply(tempfunc,itemp=1:nEpisPairLoci))    # using apply
  ...
  ### calculate statistical variances for dominance
  varbylocus <- array(data = 0,dim=c(nTrait,nTrait,nQtl))
  tempfunc <- function(itemp) {
    varbylocus[,,itemp] <<- calculateVarbyLocus(EffInput=domValues_Stat[itemp,],px=px_d1[itemp,],
                                                hx=hx_d1[itemp,])
  }
  invisible(mapply(tempfunc,itemp=1:nQtl))    # using apply
  v_temp=array(data = 0,dim=c(nTrait,nTrait))
  ...
  crit=crit+sum(abs(round(cov2cor(aRes_covar)-cov2cor(v_aStat),digits=1))*critWeightcorAbio)
  ...
  # variance
  varvectObserved <- c(diag(v_aStat),diag(v_aaStat),diag(v_dStat))
