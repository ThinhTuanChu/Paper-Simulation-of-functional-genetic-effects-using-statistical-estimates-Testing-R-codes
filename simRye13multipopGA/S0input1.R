...
# Only work for this statistical model: y = a_Res + (a_NR + a_MS) + aa_msNRRes + d_msNRRes + e. 
# Traits: Minimum of 2 traits are used. This paper uses only trait 2.
nTrait=nObs=nEbv=nRes=2
# assuming this is the additive variance of founder Pop1 (HWE).
aRes_covar <- matrix(c(500.0, 0.00,
                       0.0, 500.00),nrow = nTrait,byrow = T)
# assuming this is the additive variance of founder Pop2 (HWE).
aNRMS_covar <- matrix(c(300.0, 0.00,
                        0.0, 300.0),nrow = nTrait,byrow = T)
# AxA variance
...
critWeight <- c(0.0,1.0,  0,1.0,  0.0,1.0,  0.0,1.0)				# nTrait * 4
...
programdir="C:/ThinhPLANTS/Hybrimax/Paper1/code2GitHub/simRye13multipopGA/"
if (!dir.exists(programdir)) programdir='/usr/home/qgg/chuthinh/Hybrimax/simulation4/simRye13multipopGA/'
