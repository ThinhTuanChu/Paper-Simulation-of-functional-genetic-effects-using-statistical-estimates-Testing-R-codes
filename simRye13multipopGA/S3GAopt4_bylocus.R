...
# use domvalues instead of domdegreeValues for criteriaAccept Function
...
generateStartPop <- function(genodf=NULL,idtobeused=NULL,idfrequencyCalculation=NULL,infuncParameters='biological',nIndividuals=100,
  ...
  for (idstartpop in 1:nIndividuals) {  
    ...
    # dominance
    for (iTrait in 1:(nTrait)) {
      startpop[idstartpop,(counter+1):(counter+nQtl)] <- domValues[1:nQtl,iTrait]
      counter=counter+nQtl
    }
    # epistasis
    for (iTrait in 1:(nTrait)) {
...
startpop <- generateStartPop(genodf=geno3way,nIndividuals=nstartpop,infuncParameters='biological',methodCalculateVar='bylocus',
...
addingpop <- generateStartPop(genodf=geno3way,nIndividuals=nstartpop,infuncParameters='statisticalTwo',methodCalculateVar='bylocus',
...
# setup hx and px for a1 par1 pop calculation of variances by locus
px_a1 <- freqGenotypes_genoFounderRes[SNPinfo[,4]=='T',]
px <- lapply(seq_len(nrow(px_a1)), function(i) px_a1[i,])
hx_a1 <- array(data = 0,dim=c(nQtl,3))
hx_a1[,1] <- mapply(findhai,px=px,allelecount=0)
hx_a1[,2] <- mapply(findhai,px=px,allelecount=1)
hx_a1[,3] <- mapply(findhai,px=px,allelecount=2)

# setup hx and px for a2 par1 pop calculation of variances by locus
px_a2 <- freqGenotypes_genoFounderNRMS[SNPinfo[,4]=='T',]
px <- lapply(seq_len(nrow(px_a2)), function(i) px_a2[i,])
hx_a2 <- array(data = 0,dim=c(nQtl,3))
hx_a2[,1] <- mapply(findhai,px=px,allelecount=0)
hx_a2[,2] <- mapply(findhai,px=px,allelecount=1)
hx_a2[,3] <- mapply(findhai,px=px,allelecount=2)

# setup h1h2d & p1p2d & calculateStatVarDPlotbyLocusForGA for calculation of variances by locus for dominance
h1=c(0,1,2);h2=c(0,1,2)
...
ap1=freqAlleles_genoFounderRes[SNPinfo[,4]=='T',1]
ap2=freqAlleles_genoFounderNRMS[SNPinfo[,4]=='T',1]
p1=freqGenotypes_genoinbredRes[SNPinfo[,4]=='T',]
p2=freqGenotypes_geno2wayNRMS[SNPinfo[,4]=='T',]
h1h2d <- p1p2d <- array(data = 0,dim=c(nQtl,9))
tempfunc <- function (itemp) {
  ap1=ap1[itemp]; ap2=ap2[itemp]; p1=p1[itemp,]; p2=p2[itemp,]
  h1h2d[itemp,] <<- pa11*pa22*2*(1-ap1)*ap2 + pa12*pa21*2*ap1*(1-ap2) +
    pa11*pa21*(-2)*(1-ap1)*(1-ap2) + pa12*pa22*(-2)*ap1*ap2
  p1p2d[itemp,] <<- kronecker(p2,p1)
}
invisible(mapply(tempfunc,itemp=1:nQtl))    # using apply

calculateStatVarDPlotbyLocusForGA <- function(EffInput,h1h2,p1p2) {
...
# setup hklaa & pklaa & calculateVarPlotbyLocusEpisForGA for calculation of variances by locus for dominance
hklaa <- pklaa <- array(data = 0,dim=c(nEpisPairLoci,81))
tempfunc <- function(itemp) {
  hk1=c(0,1,2);hk2=c(0,1,2);hl1=c(0,1,2);hl2=c(0,1,2)
  apk1=freqAlleles_genoFounderRes[episPairLoci[itemp,1],1];apk2=freqAlleles_genoFounderNRMS[episPairLoci[itemp,1],1]
  pk1=freqGenotypes_genoinbredRes[episPairLoci[itemp,1],];pk2=freqGenotypes_geno2wayNRMS[episPairLoci[itemp,1],]
  apl1=freqAlleles_genoFounderRes[episPairLoci[itemp,2],1];apl2=freqAlleles_genoFounderNRMS[episPairLoci[itemp,2],1]
  pl1=freqGenotypes_genoinbredRes[episPairLoci[itemp,2],];pl2=freqGenotypes_geno2wayNRMS[episPairLoci[itemp,2],]
  hk1 = (hk1-2*(1-apk1))/2
  hk2 = (hk2-2*(1-apk2))/2
  hl1 = (hl1-2*(1-apl1))/2
  hl2 = (hl2-2*(1-apl2))/2
  ...
  hklaa[itemp,] <<- kronecker(hl,hk)
  
  pk = kronecker(pk2,pk1)
  pl = kronecker(pl2,pl1)
  pklaa[itemp,] <<- kronecker(pl,pk)
}
invisible(mapply(tempfunc,itemp=1:nEpisPairLoci))    # using apply

calculateVarPlotbyLocusEpisForGA <- function (EffInput,hkl,pkl) {
...
criteriaAccept <- function(tempSol) {  #tempSol=startpop[1,];tempSol=opt@solution
  ...
  domValues <- array(data = 0,dim=c(nQtl,nTrait))
  for (iTrait in 1:nTrait) {
    domValues[1:nQtl,iTrait] <- tempSol[(counter+1):(counter+nQtl)]
    counter=counter+nQtl
  }
  ...
  ### calculate statistical variances of par1, par2 pops and dominance contributed to plot 
  varbylocusa1 <- varbylocusa2 <- varbylocusd <- array(data = 0,dim=c(nTrait,nTrait,nQtl))
  tempfunc <- function(itemp) {
    varbylocusa1[,,itemp] <<- calculateVarbyLocus(EffInput=a1Qtleffects_Stat[itemp,],px=px_a1[itemp,], hx=hx_a1[itemp,])
	varbylocusa2[,,itemp] <<- calculateVarbyLocus(EffInput=a2Qtleffects_Stat[itemp,],px=px_a2[itemp,],hx=hx_a2[itemp,])
    varbylocusd[,,itemp] <<- calculateStatVarDPlotbyLocusForGA(EffInput=domValues_Stat[itemp,],h1h2=h1h2d[itemp,],p1p2=p1p2d[itemp,])
  }
  invisible(mapply(tempfunc,itemp=1:nQtl))    # using apply
  
  #### calculate statistical epistasis by animals for geno3way
  varbylocusaa <- array(data = 0,dim=c(nTrait,nTrait,nEpisPairLoci))
  tempfunc <- function(itemp) {
    varbylocusaa[,,itemp] <<- calculateVarPlotbyLocusEpisForGA(EffInput=episPairLociEffect_Stat[itemp,],
                                                             hkl=hklaa[itemp,], pkl=pklaa[itemp,])
  }
  invisible(mapply(tempfunc,itemp=1:nEpisPairLoci))    # using apply
  
  ### total variance
  v_a1Stat = v_a2Stat = v_dStat = v_aaStat = array(data = 0,dim=c(nTrait,nTrait))
  for (iTrait in 1:(nTrait)) {
    for (jTrait in 1:(nTrait)) {
      v_a1Stat[iTrait,jTrait]=sum(varbylocusa1[iTrait,jTrait,])
	  v_a2Stat[iTrait,jTrait]=sum(varbylocusa2[iTrait,jTrait,])
	  v_dStat[iTrait,jTrait]=sum(varbylocusd[iTrait,jTrait,])
	  v_aaStat[iTrait,jTrait]=sum(varbylocusaa[iTrait,jTrait,])
    }
  } 
