...
if (reportVarScheme) {
  ...
  # True Plot Statistical Variances
  idLst <- pop[pop$fixedFactor1>0 & pop$currentpop %in% c(4),'id']
  idLst1 <- pop[idLst,'par1']
  idLst2 <- pop[idLst,'par2']
  temp1 <- calculateStatEffectPlotOne(idPar1=idLst1,idPar2=idLst2,returnvalues='variances')
  temp1 <- data.frame(iRepScheme,pop=12,freqPop=4,temp1)
  write.table(temp1,file=filename,sep = "\t",row.names=F,col.names=!file.exists(filename),quote = FALSE,append=file.exists(filename))
}
...
################################### Different GRM ###################################
# 3way hybrid ms NR Res
idLst <- pop[pop$fixedFactor1>0 & pop$currentpop %in% c(4),'id']
gmatTemp <- calculateGmatbyRmths1(genodf=geno,idtobeused=idLst,igmatrixfile=paste0('gmatinvInbredPop',4,'.add'),methodgmat='VanRaden1.add',returngmat='gmat')
calculateEpisGmatbyRmths1(gmat1=gmatTemp,idGmat=idLst,igmatrixfile=paste0('gmatinvInbredPop',4,'.axa'),scaleDiag=FALSE)

# 3way hybrid ms NR Res
idLst <- pop[pop$fixedFactor1>0 & pop$currentpop %in% c(4),'id']
calculateGmatbyRmths1(genodf=geno,idtobeused=idLst,igmatrixfile=paste0('gmatinvInbredPop',4,'.dom'),methodgmat='Moghaddar.dom')

#################################### Running DMU #####################################
#Create DMU dir file
filename=paste0('ipop4Mod101','rep',iRepScheme,'.DIR')
dmufile=substring(filename, 1, last = (nchar(filename)-4))
write(paste('$COMMENT ADAM-DMU interface'),file=filename,append=F)
write(paste('y = mean + hys + msNRRes_a + msNRRes_aa + msNRRes_d  + e'),file=filename,append=T)
...
write(paste('0'),file=filename,append=T)
write(paste('0'),file=filename,append=T)
write(paste('$VAR_STR 1 GREL ASCII', paste0('gmatinvInbredPop',4,'.add')),file=filename,append=T)
write(paste('$VAR_STR 2 GREL ASCII', paste0('gmatinvInbredPop',4,'.axa')),file=filename,append=T)
write(paste('$VAR_STR 3 GREL ASCII', paste0('gmatinvInbredPop',4,'.dom')),file=filename,append=T)
...
# clear junks
clean.junk.files()
temp1 <- c(paste0('gmatinvInbredPop',4,'.add'),paste0('gmatinvInbredPop',4,'.axa'),paste0('gmatinvInbredPop',4,'.dom'),
...
filename='varipopDMUparout1.res'
...
# 3way hybrid ms NR Res
idLst <- pop[pop$fixedFactor1>0 & pop$currentpop %in% c(4),'id']
gmatTemp <- calculateGmatbyRmths1(genodf=geno,idtobeused=idLst,igmatrixfile=paste0('gmatinvInbredPop',4,'.add'),methodgmat='VanRaden1.add',returngmat='gmat',scaleDiag=TRUE)
calculateEpisGmatbyRmths1(gmat1=gmatTemp,idGmat=idLst,igmatrixfile=paste0('gmatinvInbredPop',4,'.axa'),scaleDiag=TRUE)

# 3way hybrid ms NR Res
idLst <- pop[pop$fixedFactor1>0 & pop$currentpop %in% c(4),'id']
calculateGmatbyRmths1(genodf=geno,idtobeused=idLst,igmatrixfile=paste0('gmatinvInbredPop',4,'.dom'),methodgmat='Moghaddar.dom',scaleDiag=TRUE)

#################################### Running DMU #####################################
#Create DMU dir file
filename=paste0('ipop4Mod102','rep',iRepScheme,'.DIR')
dmufile=substring(filename, 1, last = (nchar(filename)-4))
write(paste('$COMMENT ADAM-DMU interface'),file=filename,append=F)
write(paste('y = mean + hys + msNRRes_a + msNRRes_aa + msNRRes_d  + e'),file=filename,append=T)
write(paste('$ANALYSE 1 31 0 0'),file=filename,append=T)
...
## reading DMU variance output
temp1 <- read.table(paste0(dmufile,'.PAROUT'),header = F)
filename='varipopDMUparout2.res'
if (!file.exists(filename)) write(paste('iRep',paste0(paste0('val',c(1:length(temp1[,4]))),collapse =' ')),file=filename,append=F)
write(paste(iRepScheme,paste0(temp1[,4],collapse =' ')),file=filename,append=file.exists(filename))
...
# 3way hybrid ms NR Res
idLst <- pop[pop$fixedFactor1>0 & pop$currentpop %in% c(4),'id']
gmatTemp <- calculateGmatbyRmths1(genodf=geno,idtobeused=idLst,igmatrixfile=paste0('gmatinvInbredPop',4,'.add'),methodgmat='Vitezica2017.add',returngmat='gmat',scaleDiag=TRUE)
calculateEpisGmatbyRmths1(gmat1=gmatTemp,idGmat=idLst,igmatrixfile=paste0('gmatinvInbredPop',4,'.axa'),scaleDiag=TRUE)

# 3way hybrid ms NR Res
idLst <- pop[pop$fixedFactor1>0 & pop$currentpop %in% c(4),'id']
calculateGmatbyRmths1(genodf=geno,idtobeused=idLst,igmatrixfile=paste0('gmatinvInbredPop',4,'.dom'),methodgmat='Vitezica2017.dom',scaleDiag=TRUE)

#################################### Running DMU #####################################
#Create DMU dir file
filename=paste0('ipop4Mod103','rep',iRepScheme,'.DIR')
dmufile=substring(filename, 1, last = (nchar(filename)-4))
write(paste('$COMMENT ADAM-DMU interface'),file=filename,append=F)
write(paste('y = mean + hys + msNRRes_a + msNRRes_aa + msNRRes_d  + e'),file=filename,append=T)
write(paste('$ANALYSE 1 31 0 0'),file=filename,append=T)
...
## reading DMU variance output
temp1 <- read.table(paste0(dmufile,'.PAROUT'),header = F)
filename='varipopDMUparout3.res'
if (!file.exists(filename)) write(paste('iRep',paste0(paste0('val',c(1:length(temp1[,4]))),collapse =' ')),file=filename,append=F)
write(paste(iRepScheme,paste0(temp1[,4],collapse =' ')),file=filename,append=file.exists(filename))
