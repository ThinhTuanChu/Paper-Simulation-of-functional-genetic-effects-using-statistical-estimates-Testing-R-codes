...
for (ichrom in 1:nChr) {
  ...
  #	chrom[[ichrom]]$positionsnp = SNPinfo[SNPinfo[,2]==ichrom,3]
  #	chrom[[ichrom]]$freqAllelesFounderbypop = array(data = 0,dim=c(chrom[[ichrom]]$nSnpEachChr,nAlleles,nFounderPop))
  #	chrom[[ichrom]]$freqAllelesFounderAllPop = array(data = 0,dim=c(chrom[[ichrom]]$nSnpEachChr,nAlleles))
  #	chrom[[ichrom]]$aQtleffects <- array(data = 0,dim=c(chrom[[ichrom]]$nQtlEachChr,nAlleles,nTrait*nFounderPop))
  #	chrom[[ichrom]]$domdegreeValues <- array(data = 0,dim=c(chrom[[ichrom]]$nQtlEachChr,nAlleles,nAlleles,nTrait*nFounderPop))
  #	chrom[[ichrom]]$domValues <- array(data = 0,dim=c(chrom[[ichrom]]$nQtlEachChr,nAlleles,nAlleles,nTrait*nFounderPop))
  #	chrom[[ichrom]]$snpNoOld <- SNPinfo[SNPinfo[,2]==ichrom,'snpNo']         # snp number on haplotype file (genotype file) originally.
  #	chrom[[ichrom]]$snpNo <- c((counter+1):(counter+chrom[[ichrom]]$nSnpEachChr))         # snp number on haplotype file (genotype file), currently
  #	counter=counter+chrom[[ichrom]]$nSnpEachChr
}
...
for (iPop in 1:nFounderPop) {
  Poptemp <- read.table(baseHaplotypesFile, header = F, skip =n, nrow = nBaseHap)
  Poptemp <- as.matrix(Poptemp); Poptemp <- Poptemp[,-1]
  #Poptemp <- Poptemp - 1  # code Qtl as 0 or 1, not correct if 4 Qtl.
  Poptemp <- Poptemp[,SNPinfo$snpNo]
...
#if (sum(abs(episcovarTraits))>0.00001) {
