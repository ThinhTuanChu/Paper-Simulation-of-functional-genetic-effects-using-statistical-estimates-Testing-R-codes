...
#nChr <- 7; nMarkerEachChr <- c(rep(2,nChr)); nQtlEachChr <- c(rep(50,nChr))   # nQtlEachChr <- NULL;  #  Testing input
nFounderPop=2
nFounderAnimal=100
nselfing=4
DMUavailable <- FALSE
...
programdir="C:/ThinhPLANTS/Hybrimax/Paper1/code2GitHub/simRye13LEpop/"
if (!dir.exists(programdir)) programdir='/usr/home/qgg/chuthinh/Hybrimax/simulation4/simRye13LEpop/'
nQtlEachQtlInteractwith=1
methodEpistasis='alphaSim'
nfixedFactors=2
nranFactors=1
inputParameters <- 'biological'   #  'statisticalOne'
domdegreevar <- domdegreemean <- NULL
varResiduals <- diag(rep(100,nRes),nrow = nRes)  # Residual variance
